...
export const dataview: CaseStudy = {
  ...
  teaser:
    "Six months to ship sorting, filtering, server pagination, column pinning, sub-rows, and bulk actions — then a CRM feedback loop and a 200-row performance fix that landed in PR 21060.",
  workRole: "Design system lead",
  metrics: [
    { label: "Build time", value: "~6 months" },
    { label: "Row perf fix", value: "PR 21060" },
    { label: "CRM loop", value: "Production" },
  ...
  sections: [
    {
      ...
      images: [
        {
          label: "Side-by-side: three bespoke table implementations vs DataView",
          aspect: "pair",
        },
      ],
    },
    ...
    {
      heading: "CRM as the feedback loop",
      body: "CRM was the primary consumer and the harshest reviewer. Nico, Tej, and Richard knew workflows I was still learning. They did not care about component elegance. They cared whether an operator could scan two hundred rows, pin the status column, filter by date range, expand a row for detail, select twelve records, and apply a bulk action without the UI stuttering.\n\nI treated CRM tickets as the backlog. When a pattern repeated across screens — filter chip placement, loading skeleton density, error retry on failed fetch — it went into the shared API instead of a CRM-only fork. That loop is why DataView feels opinionated. It is not abstract table theory. It is what operators actually do eight hours a day.",
    },
    {
      ...
      images: [
        {
          label: "Profiler before/after PR 21060 at 200 rows",
          aspect: "figure",
        },
      ],
    },
    ...
    },
    {
      ...
      images: [
        {
          label: "Registry contract excerpt: whenToUse / whenNotToUse for DataView",
          aspect: "figure",
        },
      ],
    },
